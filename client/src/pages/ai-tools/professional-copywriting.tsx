...
export default function ProfessionalCopywriting() {
  ...
  const [refinedStreamingText, setRefinedStreamingText] = useState("");
  const [refinedResult, setRefinedResult] = useState<any | null>(null);
  ...
  const refineForm = useForm<RefineForm>({
    defaultValues: {
      originalCopy: "",
      improvementGoals: "",
      targetAudience: "",
  ...
  // Copy Refinement Mutation
  const refineCopyMutation = useMutation({
    mutationFn: async (data: RefineForm) => {
      const { originalCopy, improvementGoals, targetAudience, toneAdjustment } = data;
      
      // Use Fast Mode with streaming if enabled
      if (fastMode) {
        abortControllerRef.current = new AbortController();
        setIsStreaming(true);
        setRefinedStreamingText("");
        setRefinedResult(null);
        
        // Get auth token before entering Promise
        ...
        return new Promise((resolve, reject) => {
          ...
          fetch('/api/copywriting/refine-fast', {
            method: 'POST',
            headers: {
          ...
          .then(response => {
            ...
            const readChunk = (): Promise<void> => {
              return reader.read().then(({ done, value }) => {
                ...
                for (const event of events) {
                  if (event.type === 'chunk') {
                    // Access event.refinedCopy directly (no secondary JSON.parse)
                    if (event.refinedCopy) {
                      setRefinedStreamingText(event.refinedCopy);
                    }
      ...
      // Quality Mode - use existing endpoint
      const response = await apiRequest('POST', '/api/copywriting/refine', {
        originalCopy,
        improvementGoals,
        targetAudience,
        toneAdjustment
    ...
    onSuccess: (result) => {
      ...
      toast({
        ...
        description: fastMode 
          ? "Lightning-fast refinement complete in 2-3 seconds!"
          : `Refined copy ready with ${result.wordCount} words`,
      });
    },
    onError: (error: any) => {
      setIsStreaming(false);
      toast({
        ...
        description: error.message || "Failed to refine copy",
        variant: "destructive",
      });
    },
  });

  const onRefineSubmit = (data: RefineForm) => {
    if (!data.originalCopy.trim()) {
      toast({
        title: "Original copy required",
        description: "Please enter the copy you want to refine",
        variant: "destructive",
      });
      return;
    }
    refineCopyMutation.mutate(data);
  };
  ...
  return (
    ...
    >
      <div className="space-y-6 sm:space-y-8">
        ...
        {/* Copy Refinement Section */}
        <DashboardCard
          title="Copy Refinement"
          description="Refine and improve existing copy with AI-powered suggestions"
          testId="card-copy-refinement"
        >
          {/* Fast Mode Toggle */}
          <div className="mb-6 p-4 bg-gradient-to-r from-primary/10 to-primary/5 border border-primary/20 rounded-lg">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <Gauge className={`w-5 h-5 ${fastMode ? 'text-primary' : 'text-slate-400'}`} />
                <div>
                  <h3 className="text-white font-semibold">
                    {fastMode ? '⚡ Fast Mode' : '🎯 Quality Mode'}
                  </h3>
                  <p className="text-sm text-slate-300">
                    {fastMode 
                      ? 'Real-time streaming, 2-3 seconds'
                      : 'Deep analysis, quality scores, 15-25 seconds'}
                  </p>
                </div>
              </div>
              <Switch
                checked={fastMode}
                onCheckedChange={setFastMode}
                data-testid="switch-fast-mode-refine"
              />
            </div>
            {fastMode && (
              <div className="mt-3 flex items-center gap-2 text-xs text-primary/80">
                <Sparkles className="w-4 h-4" />
                <span>Watch your copy being refined live!</span>
              </div>
            )}
          </div>

          <form onSubmit={refineForm.handleSubmit(onRefineSubmit)} className="space-y-6">
            <div>
              <Label htmlFor="originalCopy" className="text-white">Original Copy *</Label>
              <Textarea
                id="originalCopy"
                className="mt-2 h-32 resize-none bg-slate-800/50 border-slate-600 text-white placeholder:text-slate-400"
                placeholder="Paste your existing copy here for AI-powered refinement..."
                {...refineForm.register("originalCopy", { required: true })}
                data-testid="input-original-copy"
              />
            </div>

            <div className="grid md:grid-cols-2 gap-6">
              <div>
                <Label htmlFor="improvementGoals" className="text-white">Improvement Goals</Label>
                <Textarea
                  id="improvementGoals"
                  className="mt-2 h-24 resize-none bg-slate-800/50 border-slate-600 text-white placeholder:text-slate-400"
                  placeholder="e.g., Make it more persuasive, add urgency, simplify language..."
                  {...refineForm.register("improvementGoals")}
                  data-testid="input-improvement-goals"
                />
              </div>

              <div>
                <Label htmlFor="refineTargetAudience" className="text-white">Target Audience</Label>
                <Controller
                  name="targetAudience"
                  control={refineForm.control}
                  render={({ field: { value, onChange } }) => (
                    <Select onValueChange={onChange} value={value ?? ""}>
                      <SelectTrigger className="mt-2 bg-slate-800/50 border-slate-600 text-white" data-testid="select-refine-audience">
                        <SelectValue placeholder="Select target audience" />
                      </SelectTrigger>
                      <SelectContent>
                        {targetAudiences.map((audience) => (
            ...
            <div>
              <Label htmlFor="toneAdjustment" className="text-white">Tone Adjustment</Label>
              <Controller
                name="toneAdjustment"
                control={refineForm.control}
                render={({ field: { value, onChange } }) => (
                  <Select onValueChange={onChange} value={value ?? "maintain"}>
                    <SelectTrigger className="mt-2 bg-slate-800/50 border-slate-600 text-white" data-testid="select-tone">
                    ...
                    <SelectContent>
                      ...
                      <SelectItem value="enthusiastic">More Enthusiastic</SelectItem>
                      <SelectItem value="authoritative">More Authoritative</SelectItem>
            ...
            <Button 
              ...
              disabled={refineCopyMutation.isPending || isStreaming}
              data-testid="button-refine-copy"
            >
              {refineCopyMutation.isPending || isStreaming ? (
                <>
                  <Zap className="w-4 h-4 mr-2 animate-pulse" />
                  {isStreaming ? 'Streaming...' : 'Refining...'}
                </>
              ) : (
                <>
          ...
          {isStreaming && refinedStreamingText && (
            <div className="mt-6 space-y-4">
              <div className="flex items-center gap-2 text-primary">
                <Zap className="w-5 h-5 animate-pulse" />
                <h3 className="text-lg font-semibold text-white">Streaming Refinement...</h3>
              </div>
              <Card className="border-2 border-primary/50 bg-gradient-to-br from-primary/10 to-primary/5">
                <CardContent className="p-6">
                  <div className="bg-slate-800/30 p-4 rounded-lg border border-primary/20">
                    <p className="text-slate-100 leading-relaxed whitespace-pre-wrap">
                      {refinedStreamingText}
                    </p>
                  </div>
                </CardContent>
              </Card>
          ...
          {refinedResult && !isStreaming && (
            <div className="mt-6 space-y-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <CheckCircle className="w-5 h-5 text-green-400" />
                  <h3 className="text-lg font-semibold text-white">Refined Copy</h3>
                  {refinedResult.wordCount && (
                    <Badge className="bg-primary/20 text-primary">{refinedResult.wordCount} words</Badge>
                  )}
                </div>
                <Button
                  variant="ghost"
                  size="icon"
                  onClick={() => copyToClipboard(refinedResult.refined?.copy || refinedResult.refined, "Refined copy")}
              ...
              <div className="grid md:grid-cols-2 gap-6">
                <Card className="border-2 border-slate-600/50">
                  <CardHeader>
                    <CardTitle className="text-white text-base">Original Copy</CardTitle>
                  </CardHeader>
                  <CardContent>
                    <div className="bg-slate-800/30 p-4 rounded-lg">
                      <p className="text-slate-300 leading-relaxed whitespace-pre-wrap">
                        {refinedResult.original?.copy || refinedResult.original}
                      </p>
                    </div>
                  </CardContent>
                </Card>
                ...
                <Card className="border-2 border-primary/50 bg-gradient-to-br from-primary/10 to-primary/5">
                  <CardHeader>
                    <CardTitle className="text-white text-base">Refined Copy</CardTitle>
                  </CardHeader>
                  <CardContent>
                    <div className="bg-slate-800/30 p-4 rounded-lg border border-primary/20">
                      <p className="text-slate-100 leading-relaxed whitespace-pre-wrap">
                        {refinedResult.refined?.copy || refinedResult.refined}
                      </p>
                    </div>
                  </CardContent>
                </Card>
